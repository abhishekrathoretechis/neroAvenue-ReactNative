...
const MagzScrollScreen = props => {
  ...
  const getMagzData = (page, isFirstTime = false) => {
    setLoading(true);
    client
      .get(`magz/${ppostid}/details?page=${page}&itemSize=${itemsPerLoad}`)
      .then(res => {
        setLoading(false);
        if (res?.status === 200) {
...
export default MagzScrollScreen;
