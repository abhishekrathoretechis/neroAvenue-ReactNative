...
import OtherMagzineDetailsScreen from '../otherMagzineDetailsScreen/OtherMagzineDetailsScreen';

const OtherMagzScrollScreen = props => {
  ...
  const [ppostid, setPpostid] = useState(
    magzData?.postId ? magzData?.postId : magzData?.id,
  );
  ...
  const getMagzData = (page, isFirstTime = false) => {
    ...
    client
      .get(
        `user/${idDetails}/post/${ppostid}/all/details?type=mags&page=${page}&itemSize=${itemsPerLoad}`
      )
      .then(res => {
        setLoading(false);
        if (res?.status === 200) {
...
export default OtherMagzScrollScreen;
